...
void printSeparator(char c, int length); // New helper for UI
...
void searchBook() {
    ...
    if (!found) {
        printf("\n>>> No books found matching '%s' in the library.\n", keyword);
    }
    printf("\n");
    waitForEnter();
}
...
void issueBook() {
    ...
    printf("         ISSUE BOOK          \n");
    printf("=============================\n");
    filterBooksByStreamAndSubject(); // Reuse filtering for selection
    ...
    if (lib.streams[s].subjects[sub].books[b].quantity > 0) {
        ...
        printf("\n>>> Book '%s' issued successfully! Remaining quantity: %d\n",
            lib.streams[s].subjects[sub].books[b].name,
            lib.streams[s].subjects[sub].books[b].quantity);
    } else {
        printf("\n>>> Sorry, book '%s' is not available at the moment.\n",
            lib.streams[s].subjects[sub].books[b].name);
    }
    waitForEnter();
}
...
void returnBook() {
    ...
    lib.streams[s].subjects[sub].books[b].quantity++;
    printf("\n>>> Book '%s' returned successfully! New quantity: %d\n",
        lib.streams[s].subjects[sub].books[b].name,
        lib.streams[s].subjects[sub].books[b].quantity);
    waitForEnter();
}
