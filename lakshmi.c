...
void issueBook() {
    int streamChoice, subjectChoice, bookChoice;
    ...
    // List Books
    for (int k = 0; k < selectedSubject->bookCount; k++) {
        printf("%d. %s | Quantity: %d\n", k + 1, selectedSubject->books[k].name, selectedSubject->books[k].quantity);
    }

    printf("Select a book (1-%d): ", selectedSubject->bookCount);
    scanf("%d", &bookChoice);

    if (bookChoice < 1 || bookChoice > selectedSubject->bookCount) {
        printf("Invalid book choice.\n");
        waitForEnter();
        return;
    }

    Book* selectedBook = &selectedSubject->books[bookChoice - 1];

    if (selectedBook->quantity > 0) {
        selectedBook->quantity--;
        printf("You have successfully issued the book: %s\n", selectedBook->name);
    } else {
        printf("Sorry, the book is currently unavailable.\n");
    }
...
void viewIssuedBooks() {
    // Assuming you have a structure to track issued books
    // This function will display the list of books issued by the student
    printf("\n--- Issued Books ---\n");
    // Iterate through the list of issued books and display them
    waitForEnter();
}
